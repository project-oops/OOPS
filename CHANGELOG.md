...
- **`oops setup`**, for a machine that cannot yet build the C repositories: WSL, a build
  distribution of the collection's own called `oops-builder`, and the toolchain inside it,
  from one script that runs again harmlessly. It never touches a distribution somebody set up
  themselves, and `doctor` names it when it finds the gap.
